module.exports = {
    getLocalUsers:((req, res)=>{
        const db = req.app.get('db')
        if(req.user){
            const {user_id, latitude, longitude, distance_range, show_gender, age_min, age_max} = req.user
            db.query(`SELECT *, point(${latitude}, ${longitude}) <@> point (latitude, longitude)::point as distance_from_user
            FROM users
            ...
            (err, localUsers = res) => {}).then(localUsers => res.status(200).send(localUsers))
        }else{
